...
- `apm` is the [Agent Package Manager](https://github.com/microsoft/apm). Required for `apm install` / `apm compile`. mise has no core/registry entry for it; use the `ubi:` backend to fetch the binary from microsoft/apm GitHub releases.
- `gh` is the GitHub CLI (resolved by mise's core registry). Used by the `committing-code` skill and any PR-related workflow.
